...
@testset "More backward tests" for n in 1:64
    @testset "size: ($m, $n)" for m in n:(n + 1)
        ...
        @testset "against naive implementation" begin
            @test naive_2d_fourier_transform(X, FFTA.FFT_BACKWARD) ≈ FFTA.bfft(X)
        end

        @testset "allocations" begin
            @test (@test_allocations FFTA.bfft(X)) <= 111
        end
    end
end

@testset "error messages" begin
    @test_throws DimensionMismatch FFTA.bfft(zeros(ComplexF64, 0, 0))
end
